...
extern "C"
__global__ void __launch_bounds__(64) gemm64_3(
    int M, int N, int K,
    int LDA, int LDB,
    float* C, int LDC,
  ...
  int trackA = (gidx*16 + idx) + idy*LDA;
  //int trackA = trackA0 + 8;
  int trackB = (gidy*16 + idx) + idy*LDB;
  //int trackB8 = trackB0 + 8;
  
  float* lAstore = lA + idy*64 + idx*4;
  float* lBstore = lB + idy*64 + idx*4;

  reinterpret_cast<float4*>(lAstore + lAstart +  0)[0] = tex1Dfetch(texA, trackA);
  reinterpret_cast<float4*>(lAstore + lAstart + 32)[0] = tex1Dfetch(texA, trackA+8);
  reinterpret_cast<float4*>(lBstore + lBstart +  0)[0] = tex1Dfetch(texB, trackB);
  reinterpret_cast<float4*>(lBstore + lBstart + 32)[0] = tex1Dfetch(texB, trackB+8);
